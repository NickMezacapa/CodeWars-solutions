...
function reduceLength(e) {
	return e.length > 30
		? e
				.split("-")
				.filter((a) => !~ignores.indexOf(a))
				.map((a) => a[0])
				.join("")
				.toUpperCase()
		: e.replace(/-/g, " ");
}
function anchor(e, path) {
	path = e == "home" ? "/" : "/" + path.slice(1).join("/") + "/";
	return '<a href="' + path + '">' + reduceLength(e).toUpperCase() + "</a>";
}
function span(e) {
	return (
		'<span class="active">' +
		/[^\.]+/.exec(reduceLength(e))[0].toUpperCase() +
		"</span>"
	);
}
function generateBC(url, separator) {
	...
	url = url
		...
		// Change to array and filter out empty strings
		.split("/")
		.filter((a) => a != "")
	...
	url = url.map(function (e, i) {
		return i == url.length - 1 ? span(e) : anchor(e, url.slice(0, i + 1));
	});
